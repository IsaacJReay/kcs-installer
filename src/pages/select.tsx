...
const Input = () => {
    ...
    //     console.log("Main", MainStorage);
    //     console.log("Content", ContentStorage);
                    ...
                    <div className="flex flex-wrap -mx-3 mb-6">
                        <div className="w-full px-3">
                            ...
                            <div className="relative">
                                <select value={MainStorage} onChange={handleMainStorageChange} className="block appearance-none w-full bg-gray-100 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="grid-state">
                                    <option value=""></option>
                                    {data.map((option_data) => { return <option value={option_data.name}>{option_data.info}</option> })}
                                </select>
                                <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700">
                                    <svg className="fill-current h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M9.293 12.95l.707.707L15.657 8l-1.414-1.414L10 10.828 5.757 6.586 4.343 8z" /></svg>
                                </div>
                            </div>
                            {MainStorageMissing && <p className="text-red-600 text-xs italic">Main Storage hasn't been set yet</p>}
                        </div>
                    </div>
                    <div className="flex flex-wrap -mx-3 mb-6">
                        <div className="w-full px-3">
                            ...
                            <div className="relative">
                                <select value={ContentStorage} onChange={handleContentStorageChange} className="block appearance-none w-full bg-gray-100 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="grid-state">
                                    <option value=""></option>
                                    {data.map((option_data) => { return <option value={option_data.name}>{option_data.info}</option> })}
                                </select>
                                <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700">
                            ...
                            {ContentStorageMissing && <p className="text-red-600 text-xs italic">Content Storage hasn't been set yet</p>}
                        </div>
                    </div>
                    <div className="flex flex-wrap -mx-3 mb-6">
                        <div className="w-full px-3">
                            <label className="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" htmlFor="grid-ip">
                                Master IP Address
                            </label>
                            <input value={IPAddr} onChange={handleIPAddrChange} className="appearance-none block w-full bg-gray-100 text-gray-700 border border-gray-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
                                id="grid-password"
                                type="text"
                                placeholder="192.168.1.1"
                            />
                            {IPAddrMissing && <p className="text-red-600 text-xs italic">Master IP Address hasn't been set yet</p>}
                        </div>
                    </div>
                    <div className="flex flex-wrap -mx-3 mb-6">
